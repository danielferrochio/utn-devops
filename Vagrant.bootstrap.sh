...
if [ -f "/tmp/g2.site.conf" ]; then
	echo "Copio el archivo de configuracion de apache"
	sudo mv /tmp/g2.site.conf /etc/apache2/sites-available
	# activo el nuevo sitio web
	sudo a2ensite g2.site.conf
...
if [ ! -d "$APP_PATH" ]; then
	...
	cd $APACHE_ROOT
	sudo git clone https://github.com/danielferrochio/utn-devops-app.git
	cd $APP_PATH
